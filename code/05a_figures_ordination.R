...
load(file = "output_data/sync/03_sync_data_ordination_traitgroup_similarity_euclidean_dist_interpolated.RData") ## syncDF
head(SyncBasin)
...
sm1 <- ggplot(subset(SyncBasin, Connectivity == 1), aes(x=Euclid_Dist_KM, y=Correlation, color = Axis)) +
...
file.name1 <- paste0(out.dir, "Within_basin_sync_ordination.jpg")
ggsave(sm1, filename=file.name1, dpi=300, height=5, width=6)

## between basin synchrony

sm2 <- ggplot(subset(SyncBasin, Connectivity == 0), aes(x=Euclid_Dist_KM, y=Correlation, color = Axis)) +
  geom_smooth(method = "gam") +
...
file.name2 <- paste0(out.dir, "Between_basin_sync_ordination.jpg")
...
sm1a <- ggplot(subset(SyncBasin600, Connectivity == 1), aes(x=Euclid_Dist_KM, y=Correlation, color = Axis)) +
...
file.name1 <- paste0(out.dir, "Within_basin_sync_ordination_distance_lm_600_logx.jpg")
ggsave(sm1a, filename=file.name1, dpi=300, height=5, width=6)


## between basin synchrony

sm2a <- ggplot(subset(SyncBasin, Connectivity == 0), aes(x=Euclid_Dist_KM, y=Correlation, color = Axis)) +
  geom_smooth(method = "lm") +
  # facet_grid(cols = vars(TraitGroup), rows = vars(FeedingGroup)) +
  facet_wrap(~TraitGroup) +
  scale_y_continuous(name="Synchrony") +
...
file.name2 <- paste0(out.dir, "Between_basin_sync_ordination_distance_lm_logx.jpg")
...
MeanSync600 <- SyncBasin600 %>%
  group_by(Axis, Site_ID2, Connectivity, TraitGroup) %>%
  summarise(Mean_Cor = mean(Correlation), Mean_Dist = mean(Euclid_Dist_KM), Mean_Lat =mean(MeanLat))

s1<- ggplot(MeanSync600, aes(x=Mean_Dist, y=Mean_Cor, color = Axis)) + 
  geom_point() +
  facet_wrap(~TraitGroup) +
  scale_y_continuous(name="Synchrony") +
  scale_x_log10(name="Log Eucliean Distance (km)")
...
file.name1 <- paste0(out.dir, "Mean_within_basin_sync_ordination_points.jpg")
...
## between basin
s2 <- ggplot(subset(MeanSync, Connectivity == 0), aes(x=Mean_Dist, y=Mean_Cor, color = Axis)) + 
  geom_point() +
  facet_wrap(~TraitGroup) +
  scale_y_continuous(name="Synchrony") +
  scale_x_log10(name="Log Eucliean Distance (km)")
...
file.name1 <- paste0(out.dir, "Mean_between_basin_sync_ordination_points.jpg")
...
t1 <- ggplot(subset(SyncBasinTF, Connectivity == 1), aes(x=TempCor, y=Correlation, color = Axis)) +
  geom_smooth(method = "lm") +
  facet_wrap(~TraitGroup) +
  scale_y_continuous(name="Synchrony") +
...
t2 <- ggplot(subset(SyncBasinTF, Connectivity == 0), aes(x=TempCor, y=Correlation, color = Axis)) +
  geom_smooth(method = "lm") +
  facet_wrap(~TraitGroup) +
  scale_y_continuous(name="Synchrony") +
  scale_x_continuous(name="Synchrony of Max Temp") 
...
f1 <- ggplot(subset(SyncBasinTF, Connectivity == 1), aes(x=FlowCor, y=Correlation, color = Axis)) +
  geom_smooth(method = "lm") +
  facet_wrap(~TraitGroup) +
  scale_y_continuous(name="Synchrony") +
...
f2 <- ggplot(subset(SyncBasinTF, Connectivity == 0), aes(x=FlowCor, y=Correlation, color = Axis)) +
  geom_smooth(method = "lm") +
  facet_wrap(~TraitGroup) +
  scale_y_continuous(name="Synchrony") +
  scale_x_continuous(name="Synchrony of Max Q") 
...
SyncBasinTF_wide <- SyncBasinTF %>%
  ...
  pivot_longer(TempCor:Y, names_to="Variable", values_to = "Synchrony")
...
file.name1 <- paste0(out.dir, "Within_basin_ordination_sync_temp_flow_lm_log.jpg")
...
file.name1 <- paste0(out.dir, "Between_basin_ordination_sync_temp_flow_lm_log.jpg")
...
SyncBasinTF_wide <- SyncBasinTF %>%
  dplyr::select(-c(Axis:Correlation.y)) %>%
...
load(file = "output_data/sync/04_sync_ordination_LOO.RData") # ssLOO_join
...
smbx <- ggplot(subset(SyncBasin, Connectivity == 1), aes(x=YearRemoved, y=SyncDiff, color = Axis)) +
  geom_boxplot() +
  facet_wrap(~TraitGroup) +
...
file.name1 <- paste0(out.dir, "Within_basin_ordination_traitgrp_LOO_boxplots.jpg")
ggsave(smbx, filename=file.name1, dpi=300, height=5, width=6)


## between basin

smbx2 <- ggplot(subset(SyncBasin, Connectivity == 0), aes(x=YearRemoved, y=SyncDiff, color = Axis)) +
  geom_boxplot() +
  facet_wrap(~TraitGroup) +
  theme(axis.text.x = element_text(angle = 60, hjust=1)) +
  scale_y_continuous(name="Change in Synchrony") 
