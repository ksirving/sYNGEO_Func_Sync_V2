...
air_mv_avg_roll <- read_csv(here("input_data/Env/mv_avg_roll_air_temp.csv")) %>%
  filter(
    siteid %in% unique(fish_ab$site_id),
...
air_annual_and_summer_avg <- air_mv_avg_roll %>%
  left_join(site_desc_loc %>% select(siteid, country), by = "siteid") %>%
  mutate(
    year = year(date),
    month = month(date)
    ) %>%
...
## Test annual avg:
stopifnot(mean(air_mv_avg_roll[mask, ][["mv_avg_12m"]]) ==
  air_annual_and_summer_avg[mask_annual_summer_avg, ][["annual_avg"]])
## Test summer avg:
stopifnot(air_mv_avg_roll[mask, ][["mv_avg_3m"]][7] ==
